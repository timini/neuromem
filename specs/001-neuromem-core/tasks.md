...
- [ ] T001 Create repo-root workspace `pyproject.toml` with `[tool.uv.workspace]` declaring `members = ["packages/*"]`. Add shared `[tool.ruff]` config (line-length, target-version = "py310") and `[tool.pytest.ini_options]` (minimum-version, testpaths pointing at `packages/*/tests`). NO `[project]` table — the root is a workspace, not a package.
